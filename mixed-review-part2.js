...
function lunch() {
  ...
  // Your code here
  let randomCafeNum = randomInt(1, 4);
  if (randomCafeNum == 1) {
    return cafes[0];
  } else if (randomCafeNum == 2) {
    return cafes[1];
  } else if (randomCafeNum == 3) {
    return cafes[2];
  } else if (randomCafeNum == 4) {
    return cafes[3];
  }
}
...
function football() {
  let score = 0;
  let wayPointsObtained = readline.question("Type touchdown or field goal: ").toLowerCase();

  if (wayPointsObtained == "touchdown" || wayPointsObtained == "td") {
    score += 7;
    wayPointsObtained = readline.question("Type touchdown or field goal: ").toLowerCase();
  }

  if (wayPointsObtained == "field goal" || wayPointsObtained == "fg") {
    score += 3;
    wayPointsObtained = readline.question("Type touchdown or field goal: ").toLowerCase();
  }

  if (wayPointsObtained != "touchdown" || wayPointsObtained != "td" || wayPointsObtained != "field goal" || wayPointsObtained != "fg") {
    console.log(score);
    return;
  }
}
...
function repeatedLetter() {
  ...
  for (let i = 0; i < words.length; i++) {
    let numLetters = words[i];
    if (numLetters.length >= 3) {
      realWords.push(numLetters[i]);
    }
  }
